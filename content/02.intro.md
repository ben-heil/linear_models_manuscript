...
Unlike purely linear models such as logistic regression, non-linear models can learn nonlinear decision boundaries to differentiate phenotypes.
...
However, when we remove any linear separability from the data, we find non-linear models are still able to make useful predictions even when the linear models previously outperformed the nonlinear ones.
